<?php
/*
Plugin Name: Zendy Speed: Browser Caching
Plugin URI: https://kauai.zendy.net
Description: Leverage Browser Caching; improve your website performance and your performace grade on YSlow, Google PageSpeed, and Pingdom.
...
register_activation_hook( __FILE__, 'zendy_speed_browser_caching_install' );
register_deactivation_hook( __FILE__, 'zendy_speed_browser_caching_uninstall' );
...
if( !function_exists( 'zendy_speed_browser_caching_install' ) ) {
	function zendy_speed_browser_caching_install() {
		zendy_speed_browser_caching_install_htaccess();
	}
}
...
// Right now there is only one
// TODO: add notification about backup files that are automatically created in case the htaccess file is corrupted
if( !function_exists( 'zendy_speed_browser_caching_uninstall' ) ) {
	function zendy_speed_browser_caching_uninstall() {
		zendy_speed_browser_caching_uninstall_htaccess();
	}
}

// Update htaccess
if( !function_exists( 'zendy_speed_browser_caching_install_htaccess' ) ){
	
	function zendy_speed_browser_caching_install_htaccess(){
	
		// Keep track of operation status
		$is_install_ok = true;
		
		// We don't mess around with .htaccess
		$backup_filename = 'zendy_speed_browser_caching_install_backup' . time() . '.htaccess';
		...
		if( $is_install_ok ){
			...
			// Erase backup
			zendy_speed_browser_caching_erase_file(false, $backup_filename);
			
		}

		// Log whether the plugin was installed successfully
		if($is_install_ok) {
			update_option( 'zendy_speed_browser_caching_status', 1);
		} else {
			update_option( 'zendy_speed_browser_caching_status', 0);
		}
	...
	function write_htaccess_browser_caching_directives( $file_path ){
		...
		if (is_writable($file_path)) {
			...
			if (flock($file_handle, LOCK_EX)) {
				...
				fwrite($file_handle, "\n");
				fwrite($file_handle, "# START - Zendy Speed - Browser Caching\n");
				...
				fwrite($file_handle, "# END - Zendy Speed - Browser Caching\n");
				fwrite($file_handle, "\n");
				fflush($file_handle);
...
if( !function_exists('zendy_speed_browser_caching_uninstall_htaccess') ){	
	
	function zendy_speed_browser_caching_uninstall_htaccess() {
	
		// Don't mess with htaccess files; make a backup
		$backup_filename = 'zendy_speed_browser_caching_uninstall_backup' . time() . '.htaccess';
		...
		// If it works
		if( zendy_speed_browser_caching_remove_htaccess_directives($backup_filename) ) {
...
if( !function_exists('zendy_speed_browser_caching_remove_htaccess_directives') ){	

	function zendy_speed_browser_caching_remove_htaccess_directives($backup_filename){
		...
		if($is_operation_ok) {
			...
			if (flock($file_handle, LOCK_EX)) {
				...
				foreach($lines as $line) {
					...
					if(strpos($line, 'START - Zendy Speed - Browser Caching') !== false) {
					...
					if(strpos($line, 'END - Zendy Speed - Browser Caching') !== false) {
